...
| P.3 | HDFS-17505 (storage-policy propagation) | `P.3-TBD` | Resolved w/o code | Implement recursive storage-policy propagation so BR/IBR no longer needs IIP lock |
...
| P.6 | HDFS-17507 (symlink under FGL) | `P.6-TBD` | Resolved w/o code | **Landed on `HDFS-17386-fork` (2026-05-11).** Replaced the `PilotEnvelopeMissException` raised on ancestor symlinks with a direct `UnresolvedPathException`, matching trunk's `FSPermissionChecker.checkNotSymlink` contract. Client's `FileSystemLinkResolver` retries transparently with the resolved path — no global-lock fallback round-trip. 7 new unit + 2 new integration tests; full FGL_IIP suite (267 tests) green; `TestSymlinkHdfsFileSystem` (72 tests) unchanged. |
