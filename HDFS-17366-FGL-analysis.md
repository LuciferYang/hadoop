...
| P.2 | HDFS-17473 (QuotaLocks) | `P.2-TBD` | Resolved w/o code | **Landed on `HDFS-17386-fork` (2026-05-12).** Implemented Solution 2 from the JIRA: a `QuotaLocks` pool in `org.apache.hadoop.hdfs.server.namenode.fgl` that hands out a lazy `ReentrantLock(true)` per quota-bearing `INode` (keyed by INode ID). `FSDirectory.updateCount` / `updateCountForDelete` / the `QuotaDelta`-style `updateCount` now wrap their verify-then-update critical sections in `try-with-resources` acquisition of all quota-bearing ancestors' locks in ascending INode-ID order. `ancestorsAllowCreate` / `ancestorsAllowMutate` no longer reject quota ancestors. 3 new integration tests in `TestFSNamesystemFGLIIP` (mkdirs, namespace-usage tracking, namespace-quota-exceeded still throws), 5 new unit tests in `TestQuotaLocks` (lazy creation, reentrance, contended serialisation). Full FGL_IIP suite (159 tests), TestQuotaByStorageType (23), TestBlockStoragePolicy (25), TestDFSRename (3) green. TestQuota regresses on the same `testRename` case carried over from P.3. |
